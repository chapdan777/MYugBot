...
export const OrdersQueries = {
  ...
  getOrdersBase: (extraCondition?: string) => `
    SELECT DISTINCT O.ID, O.ITM_ORDERNUM, O.ORDERNUM, O.ORDER_TYPE, O.MANAGER, 
           ...
           (O.ORDER_TOTAL_COST - COALESCE(O.ORDER_PAY, 0)) AS ORDER_DEBT,
           O.ORDER_GENERALSQ,
  ...
  getOrdersByIds: (orderIds: number[]) => `
    SELECT
      O.ID,
      COALESCE(C.CLIENTNAME, O.CLIENT) as CLIENTNAME,
      (SELECT FIRST 1 J.BOX_COUNT FROM JOURNAL_OUT J WHERE J.ORDER_ID = O.ID) as BOX_COUNT,
      O.ORDER_TOTAL_COST,
      O.ORDER_PAY,
      O.ORDER_STATUS,
      (O.ORDER_TOTAL_COST - COALESCE(O.ORDER_PAY, 0)) AS ORDER_DEBT
