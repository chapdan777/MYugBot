...
export class OrdersService {
  ...
  async getOrderElements(orderId: number): Promise<OrderElement[]> {
    try {
      console.log(`Сервис заказов: запрос элементов для заказа №${orderId}`);
      const elements = await this.ordersRepository.getOrderElements(orderId);
      console.log(`Сервис заказов: получено ${elements.length} элементов для заказа №${orderId}`);
      return elements;
    } catch (error) {
      console.error(`Сервис заказов: ошибка получения элементов для заказа №${orderId}:`, error);
      throw error;
    }
  }
  ...
  formatOrderElementsForDisplay(elements: OrderElement[]): string {
    ...
    elements.forEach((el, index) => {
      // Экранируем специальные HTML символы в названии элемента
      const escapedName = this.escapeHtml(el.name);
      text += `${index + 1}. <b>${escapedName}</b>`;
