...
module.exports = {
  ...
  // Используется для кастомных запросов из Node-RED flows
  executeQuery: async (query, dbName = 'cubic') => {
    const { withCubic, withITM } = require('../connection');
    const connectionFn = dbName.toLowerCase() === 'itm' ? withITM : withCubic;
    
    return connectionFn(async (db) => {
      return new Promise((resolve, reject) => {
